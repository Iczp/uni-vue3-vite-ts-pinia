<template>
  <AppPage
    :title="title"
    statusBarBackgroundColor="#C8161D"
    titleBackgroundColor="#C8161D"
    titleColor="white"
  >
    <van-popup
    ...
    >
      ...
      <van-button type="default" class="scan" @click="onScan"
        >扫一扫</van-button
      >
      ...
      <div class="status">
        <p>
          键盘是否可见: <strong>{{ isKeyboardVisible ? "是" : "否" }}</strong>
        </p>
        <p>
          键盘实时高度: <strong>{{ keyboardHeight.toFixed(2) }} px</strong>
        </p>
        <p>
          当前屏幕方向:
          <strong>{{
            isLandscape ? "横屏 (Landscape)" : "竖屏 (Portrait)"
          }}</strong>
        </p>
      </div>
